...
Limitations
-----------
* QuickLook struggles with `.mp4` files. In particular, QuickLook on recent versions of OSX won't let this plugin preview `.mp4` files that it can't handle itself (e.g containing H.265 video) or you that you have asked it not to handle via the `SnapshotAlways` [setting](#customisation). Rename your `.mp4` files as `.m4v`, or use another container for H.265 content - e.g. [Matroska](http://www.matroska.org/).
